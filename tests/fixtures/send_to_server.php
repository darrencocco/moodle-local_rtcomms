...
$PAGE->set_url('/local/rtcomms/tests/fixtures/send_to_server.php');
...
$PAGE->requires->js_amd_inline(<<<EOL
    ...
    require(['jquery', 'local_rtcomms/api'], function($, RealTimeAPI) {
        $('body').on('click', '.testform', function(e) {
            e.preventDefault();
            RealTimeAPI.sendToServer({$usercontextid}, 'local_rtcomms', 'test', 0,
                {'data': $(e.currentTarget).data('linkid')});
        });
